...
// NotifyRoles
const ReactionRoleMessageIDs = [ '708594329588858881', '803224102881591326', '815891176880078869' ];
...
client.on('raw', async (evt) => {
    ...
    if ( !fetchedSlashCommand ) {
        await SlashModule.CallbackEphemeral(data, `Sorry ${GuildMember.displayName} - something prevented me from executing the **${CommandData.name}** command...`);
        return;
    }
    else {
            ...
            if ( GuildMember.user.id === "156482326887530498" ) {
                ...
                if ( now < expirationTime ) {
                    ...
                    if ( timeLeft >= 60 && timeLeft < 3600 ) {
                        timeLeft /= 60;
                        return await SlashModule.CallbackEphemeral(data, `${GuildMember.displayName} - Please wait ${timeLeft.toFixed(1)} more minutes before using the \`${fetchedSlashCommand.name}\` command`);
                    }
                    // Hours
                    else if ( timeLeft >= 3600 ) {
                        timeLeft /= 3600;
                        return await SlashModule.CallbackEphemeral(data, `${GuildMember.displayName} - Please wait ${timeLeft.toFixed(1)} more hours before using the \`${fetchedSlashCommand.name}\` command`);
                    }
                    // Seconds
                    else {
                        return await SlashModule.CallbackEphemeral(data, `${GuildMember.displayName} - Please wait ${timeLeft.toFixed(1)} more seconds before using the \`${fetchedSlashCommand.name}\` command`);
                    }
        ...
        try {
            ...
            await SlashModule.CallbackEphemeral(data, `Sorry ${GuildMember.displayName} - there was an error trying to run that command...`);
        }
